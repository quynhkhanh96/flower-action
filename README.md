...
```shell 
python -m datasets.frame_dataset --n_clients=4 --data_dir $DATA_DIR --train_ann=$TRAIN_ANNOTATION_PATH --val_ann=$VAL_ANNOTATION_PATH
```

Start the server:
```shell 
SERVER_DIR="/ext_data2/comvis/khanhdtq/afosr2022/fed_exps/10052022"
CUDA_VISIBLE_DEVICES=3 python -m video_server --server_address=$SERVER_ADDRESS --cfg_path=$CFG_PATH --data_dir=$DATA_DIR --work_dir=$SERVER_DIR
```

Start the client:
```shell 
CUDA_VISIBLE_DEVICES=1 python -m video_client --server_address=$SERVER_ADDRESS --cid=0 --cfg_path=$CFG_PATH --data_dir=$DATA_DIR 
CUDA_VISIBLE_DEVICES=2 python -m video_client --server_address=$SERVER_ADDRESS --cid=1 --cfg_path=$CFG_PATH --data_dir=$DATA_DIR
CUDA_VISIBLE_DEVICES=2 python -m video_client --server_address=$SERVER_ADDRESS --cid=2 --cfg_path=$CFG_PATH --data_dir=$DATA_DIR
