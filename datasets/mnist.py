...
def mnist_partition(n_clients, classes_per_client, working_dir):
...
def get_mnist_client_loader(client_id, local_bz, test_bz, working_dir):
    ...
    client_dst = MNISTDataset(client_images, client_labels, transforms_train)
    client_dl = DataLoader(client_dst, batch_size=local_bz,
                    shuffle=True, num_workers=2
    )

    test_dl = DataLoader(MNISTDataset(x_test, y_test, transforms_eval),
                    batch_size=test_bz, shuffle=False
    )

    return client_dl, test_dl

def get_mnist_test_loader(test_bz):
    data_test = torchvision.datasets.MNIST(DATA_PATH, train=False, download=True)
    x_test, y_test = data_test.data.numpy().reshape(-1, 1, 28, 28) / 255, np.array(data_test.targets)
...
if __name__ == '__main__':
    ...
    mnist_partition(n_clients=cfgs['num_C'], classes_per_client=cfgs['Nc'], 
