...
def get_grads(kernel_grad, kernel, w_p, args):
    ...
    return w_p * a * kernel_grad + w_p * b * kernel_grad + 1.0 * c * kernel_grad, \
            (a * kernel_grad).sum()
...
def ternary_train(model, loss, optimization_step_fn, train_iterator, args):
    """
...
def fed_ttq(network, pre_model, train_iter, args):
    ...
    model_dict, wp_lists = ternary_train(model, loss_fun, optimization_step_fn, train_iter, args)
